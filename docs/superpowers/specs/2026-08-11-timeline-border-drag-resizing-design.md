...
Pointer down on the right-edge handle takes precedence over tile selection and reorder handlers. It prevents the default native drag, stops propagation, captures the pointer, records the active pointer ID, and records the tile's original hold and horizontal coordinate. Pointer move, pointer up, and pointer cancellation ignore every pointer ID except the active one. Commit, cancellation, rejection, and disposal release pointer capture when the handle is still connected and owns that capture; release failures are swallowed after transient state has been restored.
...
The handle uses button semantics because the hold has no arbitrary maximum. Its accessible label includes the current value and instructions, such as `Resize keyframe 2 hold: 4 frames, 0.17 seconds. Use Left and Right arrows; Home sets one frame.` Left/Right arrow keys commit a one-frame decrease or increase through the same absolute-hold callback; Home commits a hold of one frame; Enter and Space perform the normal button activation of increasing by one frame. Every handled key prevents the browser default and stops propagation so it cannot scroll the timeline or trigger a Studio shortcut. A synthesized or pointer `click` on the handle is consumed without another mutation because pointer up or key down already owns activation.
...
- Tests verify a resize start and its synthesized click do not trigger tile selection, duplicate a mutation, or start reorder behavior.
...
- Keyboard tests verify Left/Right, Home, Enter, and Space use the defined absolute holds, respect the one-frame minimum, suppress defaults/propagation, and restore focus deterministically.
...
- Manual browser checks expand and shrink final and non-final tiles while the timeline is horizontally scrolled in both Studios.
