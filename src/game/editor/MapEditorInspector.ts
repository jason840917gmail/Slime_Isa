...
function resourceTagOptions(selectedTag: string): string {
  const legacyOption = selectedTag && !isResourceTag(selectedTag)
    ? `<option value="${escapeHtml(selectedTag)}" selected>${escapeHtml(selectedTag)} (unconfigured)</option>`
    : '';
  return `<option value="" ${selectedTag ? '' : 'selected'}>No requirement</option>${legacyOption}${RESOURCE_TAGS.map((tag) => `<option value="${escapeHtml(tag)}" ${tag === selectedTag ? 'selected' : ''}>${escapeHtml(tag)}</option>`).join('')}`;
}
...
function renderGameplayAttributes(state: GameplayAttributeViewState, instance?: EditableObjectInstance): string {
  ...
  return `<form class="editor-gameplay-form" data-gameplay-form>
    ...
    <section class="editor-inspector-section"><div class="editor-inspector-section-title"><span>03</span><h3>Effects and requirements</h3></div>
      ...
      <div class="editor-inspector-grid"><label class="editor-inspector-field"><span>Required tool tag</span><select data-gameplay-field="harvestTargetTag">${resourceTagOptions(resource.harvestTargetTag)}</select>${error('harvestTargetTag')}</label><label class="editor-inspector-field"><span>Minimum tool tier</span><input type="number" min="1" step="1" data-gameplay-field="harvestMinimumTier" value="${resource.harvestMinimumTier}" />${error('harvestMinimumTier')}</label></div>
