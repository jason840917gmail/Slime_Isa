...
export function renderLayeredWeaponHitboxControls(hitboxId: string, hitbox: WeaponHitboxDocument, disabled = false): string {
  ...
  const lockNote = disabled ? '<p class="studio-help layered-hitbox-lock-note">Inherited direction. Convert it to custom before editing this geometry.</p>' : '';
  return `<div class="layered-hitbox-editor${disabled ? ' is-inherited' : ''}">${lockNote}<div class="studio-field-grid layered-hitbox-geometry-grid"><label class="studio-field"><span>Shape<small>runtime primitive</small></span><select data-weapon-field="hitbox:${escapeHtml(hitboxId)}:shape" data-weapon-hitbox-control>${HITBOX_SHAPES.map((shape) => `<option value="${shape}" ${hitbox.shape === shape ? 'selected' : ''}>${shape}</option>`).join('')}</select></label>${geometryFields}</div><div class="studio-field-grid layered-hitbox-effects-grid">${numberField('Damage ×', hitboxId, 'damageMultiplier', hitbox.damageMultiplier ?? 1, 'multiplier', 'min="0"', '0.05')}${numberField('Knockback ×', hitboxId, 'knockbackMultiplier', hitbox.knockbackMultiplier ?? 1, 'multiplier', 'min="0"', '0.05')}</div><button type="button" class="studio-button studio-button--danger" data-action="delete-hitbox" data-hitbox-id="${escapeHtml(hitboxId)}"${disabled ? ' disabled' : ''}>DELETE HITBOX</button></div>`;
}
